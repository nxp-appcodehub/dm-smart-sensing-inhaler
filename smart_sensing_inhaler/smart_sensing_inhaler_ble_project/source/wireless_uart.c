...
#define mmpl3115IntervalInMs_c     (100)     /* Flush Timeout in Ms */
...
#define RESET_INHALER(x)                                                               \
        start_inhalation = 1;                                                          \
        meds_inhaled = 0;                                                              \
        reset_inhaler_flag = 0;                                                        \
        all_meds_inhaled = 0;                                                          \
        if (rawAccel.accel[0] > 300)                                                   \
        {                                                                              \
        	reset_inhaler_flag = 1;                                                    \
        }                                                                              \
        ...
        {                                                                              \
			BleApp_SendUartStream(&correctposture[0], 70U);                            \
            BleApp_SendUartStream(&start_inhaling[0], 70U);                            \
        }                                                                              \
...
static uint8_t inhalation_count = 0;
static uint8_t remaining_dosages = 0;
uint8_t skip_reading = 0;
uint8_t start_inhalation = 1;
bool compute_baseline_pr = false;
bool askResume   = true;

uint32_t pressureInPascals;
uint8_t meds_inhaled = 0;
uint8_t total_doses = TOTAL_DOSES;
uint8_t reset_inhaler_flag = 0;
uint8_t init_refpressure = 1;
int32_t status;
uint8_t i = 0;
uint8_t reading = 0;
uint8_t dataReady;
...
float accel[3];
static uint32_t refPressure = 0;
uint32_t inhalerTHS = INHALE_THS;
uint8_t all_meds_inhaled = false;

int mpl3115_int_BLE(void);
int mpl3115_event_BLE(void);
void mpl3115_CallBack();
void mpl3115_TimerCallback();
void apply_autozero(void);
...
static TIMER_MANAGER_HANDLE_DEFINE(mpl3115Id);
...
uint8_t vec_init[70] = 			"\r\n ISSDK mpl3115 sensor driver example to detect motion event & AWS\r\n";
uint8_t vec_i2c_init[70] = 		"\r\n I2C Initialization Failed\r\n";
uint8_t vec_power_cont[70] =	"\r\n I2C Power Mode setting Failed\r\n";
uint8_t vec_mode_cont[70] =		"\r\n I2C Control Mode setting Failed\r\n";
uint8_t vec_inint_sensor[70] =	"\r\n Sensor Initialization Failed\r\n";

uint8_t vec_fxls8974[70] =		"\r\n Successfully Initialized On-Board FXLS8974CF with WHO_AM_I = 0x86\r\n";
uint8_t vec_mpl3115[70] =		"\r\n Successfully Initialized MPL3115 with WHO_AM_I = 0xC4\r\n";
uint8_t vec_fxpq3115[70] =		"\r\n Successfully Initialized FXPQ3115 with WHO_AM_I = 0xC5\r\n";
uint8_t vec_timandra[70] =		"\r\n Successfully Initialized Timandra with WHO_AM_I = 0x86\r\n";
uint8_t vec_timandra_1[70] =		"\r\n Successfully Initialized Timandra with WHO_AM_I = 0x88\r\n";
uint8_t vec_chiron[70] =        "\r\n Successfully Initialized Chiron with WHO_AM_I = 0x83 \r\n";
uint8_t vec_chiron_1[70] =        "\r\n Successfully Initialized Chiron with WHO_AM_I = 0x81\r\n";
uint8_t vec_newstein[70] =		"\r\n Successfully Initialized Newstein with WHO_AM_I = 0x62 \r\n";
uint8_t vec_whoiam[70] =		"\r\n Bad WHO_AM_I = \r\n";

uint8_t vec_fxls8974_err[70] =	"\r\n FXLS89xx Sensor Configuration Failed\r\n";
uint8_t vec_fxls8974_succ[70] =	"\r\n Successfully Applied FXLS89xx Sensor Configuration\r\n";

uint8_t vec_sensor_err[70] =	"\r\n PL3115 Sensor Configuration Failed\r\n";
uint8_t vec_sensor_succ[70] =	"\r\n Successfully Applied PL3115 Sensor Configuration\r\n";

uint8_t vec_read_failed[70] =	"\r\n Read Failed. \r\n";

uint8_t performing_avg[70] =	"\r\n Perform averaging to get reference/baseline pressure value\r\n\r\n";
uint8_t show_avg_status[70] =	"  *";
uint8_t pressure_value[70] =	"\r\n\r\n Reference Pressure    = %d Pa\r\n\r\n";
uint8_t start_inhaling[70] =	"\r\n\r\n Start Inhaling Dosage ....\r\n";
uint8_t dose[70] =	"\r\n Inhalation Detected \r\n";
uint8_t total_inhaled[70] =	"\r\n Total Doses Inhaled = \r\n";
uint8_t remaining_doses[70] =	"\r\n Remaining Doses Before Refill = \r\n";
uint8_t filling_dosage[70] =	"\r\n Wait next dose is getting prepared\r\n";
uint8_t refill_reminder[70] =	"\r\n ===== Reminder: Refill Inhaler ===== \r\n";
uint8_t refill_dosoge[70] =	"\r\n ===== No Doses Left: Refill Inhaler ===== \r\n";
uint8_t dose1[70] =	"\r\n 1st Dose Inhaled\r\n";
uint8_t dose2[70] =	"\r\n 2nd Dose Inhaled\r\n";
uint8_t dose3[70] =	"\r\n 3rd Dose Inhaled\r\n";
uint8_t autozero1[70] =	"\r\n Inhale Dose1\r\n";
uint8_t autozero2[70] =	"\r\n Inhale Dose2\r\n";
uint8_t autozero3[70] =	"\r\n Inhale Dose3\r\n";
uint8_t alldoses[70] =	"\r\n All 3 Meds Inhaled\r\n";
uint8_t resetinhaler[70] =	"\r\n Reseting Inhaler\r\n\r\n\r\n";
uint8_t wrongposture1[70] =	"\r\n Wrong inhaler posture Move down\r\n";
uint8_t wrongposture2[70] =	"\r\n Wrong inhaler posture - Move up\r\n";
uint8_t correctposture[70] =	"\r\n Correct inhaler - posture\r\n";
...
void apply_autozero(void)
{
	while (i < NUM_AVG_SAMPLES)
	{
		...
		{ /* Loop, if new sample is not available. */
			...
			refPressure += pressureInPascals;

			i++;
		}
	}

	/*! Get the baseline/reference pressure value */
	refPressure = refPressure/NUM_AVG_SAMPLES;
        ...
        if (pMsg != NULL)
        {
            ...
            if (SerialManager_TryRead((serial_read_handle_t)s_readHandle, pMsg, mAppUartBufferSize, &bytesRead) == kStatus_SerialManager_Success)
            {
                if (bytesRead != 0U)
                {
                    ...
                    BleApp_SendUartStream(pMsg, (uint32_t)bytesRead);
                }
            }
...
int mpl3115_int_BLE(void)
    {
        ...
        BleApp_SendUartStream(&vec_init[0], 70U);
        ...
        if (ARM_DRIVER_OK != status)
        {
        	...
        	BleApp_SendUartStream(&vec_i2c_init[0], 70U);
        ...
        if (ARM_DRIVER_OK != status)
        {
        	...
        	BleApp_SendUartStream(&vec_power_cont[0], 70U);
        ...
        if (ARM_DRIVER_OK != status)
        {
        	BleApp_SendUartStream(&vec_mode_cont[0], 70U);
        ...
        if (ARM_DRIVER_OK != status)
        {
        	BleApp_SendUartStream(&vec_inint_sensor[0], 70U);
        ...
        {
        	BleApp_SendUartStream(&vec_fxls8974[0], 70U);
        ...
        if ((MPL3115_WHOAMI_VALUE == whoami))

        {
        	BleApp_SendUartStream(&vec_mpl3115[0], 70U);

        }
        else if ((FXPQ3115_WHOAMI_VALUE == whoami))
        {
             BleApp_SendUartStream(&vec_fxpq3115[0], 70U);
        ...
        {
        	BleApp_SendUartStream(&vec_fxls8974_err[0], 70U);
            return -1;
        }
    	BleApp_SendUartStream(&vec_fxls8974_succ[0], 70U);

        /*! Configure the MPL3115 sensor. */
        status = MPL3115_I2C_Configure(&mpl3115Driver, cMpl3115ConfigNormal);
        if (SENSOR_ERROR_NONE != status)
        {
        	BleApp_SendUartStream(&vec_sensor_err[0], 70U);
            return -1;
        }
    	BleApp_SendUartStream(&vec_sensor_succ[0], 70U);
...
void mpl3115_CallBack()
{
		...
		(void)TM_InstallCallback((timer_handle_t)mpl3115Id, mpl3115_TimerCallback, NULL);

        (void)TM_Start((timer_handle_t)mpl3115Id,
                    (uint8_t)kTimerModeLowPowerTimer | (uint8_t)kTimerModeSingleShot, mmpl3115IntervalInMs_c);
        status_ble = 0;
...
int mpl3115_event_BLE(void)
{
	...
	if (reset_inhaler_flag == 1)
	{
		BleApp_SendUartStream(&resetinhaler[0], 70U);
		RESET_INHALER();
	}
	...
	else
	{
        ...
        if (0x80 == (dataReady & FXLS8974_INT_STATUS_SRC_DRDY_MASK))
        {
			...
			if (ARM_DRIVER_OK != status)
			{
				BleApp_SendUartStream(&vec_read_failed[0], 70U);
				return -1;
			}
        ...
        if (rawAccel.accel[0] > 300)
        {
    		...
    		GPIO_PortSet(BOARD_INITPINS_LED_GREEN_GPIO, 1u << BOARD_INITPINS_LED_GREEN_PIN);
        	GPIO_PortSet(BOARD_INITPINS_LED_BLUE_GPIO, 1u << BOARD_INITPINS_LED_BLUE_PIN);
			BleApp_SendUartStream(&wrongposture1[0], 70U);
			RESET_INHALER();
        }
        else if (rawAccel.accel[0] < -300)
        {
    		...
    		GPIO_PortSet(BOARD_INITPINS_LED_GREEN_GPIO, 1u << BOARD_INITPINS_LED_GREEN_PIN);
        	GPIO_PortSet(BOARD_INITPINS_LED_BLUE_GPIO, 1u << BOARD_INITPINS_LED_BLUE_PIN);
			BleApp_SendUartStream(&wrongposture2[0], 70U);
			RESET_INHALER();
        }
		...
		if ((abs(refPressure - pressureInPascals)) > INHALE_THS)
		{
			inhalation_count++;
			remaining_dosages = (TOTAL_DOSES - inhalation_count);

			if ((TOTAL_DOSES - inhalation_count) < 15)
			{
				BleApp_SendUartStream(&refill_reminder[0], 70U);
			}

			if (inhalation_count == TOTAL_DOSES)
			{
				inhalation_count = 0;
				remaining_dosages = 0;
				BleApp_SendUartStream(&refill_dosoge[0], 70U);
			}

			BleApp_SendUartStream(&dose[0], 70U);
		    sprintf(result1, "%d", inhalation_count);
		    total_inhaled[25] = '[';
		    total_inhaled[26] = result1[0];
		    total_inhaled[27] = result1[1];
		    total_inhaled[28] = result1[2];
		    total_inhaled[29] = ']';
			BleApp_SendUartStream(&total_inhaled[0], 70U);
		    sprintf(result2, "%d", remaining_dosages);
		    remaining_doses[35] = '[';
		    remaining_doses[36] = result2[0];
		    remaining_doses[37] = result2[1];
		    remaining_doses[38] = result2[2];
		    remaining_doses[39] = ']';
			BleApp_SendUartStream(&remaining_doses[0], 70U);
			compute_baseline_pr = true;
			BleApp_SendUartStream(&filling_dosage[0], 70U);
		}
	}
}
