...
/**
 * @file fxls8974_drv.c
 * @brief The fxls8974_drv.c file implements the FXLS8964/67 sensor driver interfaces.
 */
...
// Global Variables
//-----------------------------------------------------------------------
uint8_t fxls8974_spiRead_CmdBuffer[FXLS8974_SPI_MAX_MSG_SIZE] = {0};
uint8_t fxls8974_spiRead_DataBuffer[FXLS8974_SPI_MAX_MSG_SIZE] = {0};
uint8_t fxls8974_spiWrite_CmdDataBuffer[FXLS8974_SPI_MAX_MSG_SIZE] = {0};
...
void FXLS8974_SPI_ReadPreprocess(void *pCmdOut, uint32_t offset, uint32_t size)
{
    ...
    uint8_t *pWBuff = fxls8974_spiRead_CmdBuffer;
    uint8_t *pRBuff = fxls8974_spiRead_DataBuffer;

    /* Formatting for Read command of FXLS8974 SENSOR. */
    ...
    *(pWBuff + 1) = 0x00;
    ...
    pSlaveCmd->size = size + FXLS8974_SPI_CMD_LEN;
...
void FXLS8974_SPI_WritePreprocess(void *pCmdOut, uint32_t offset, uint32_t size, void *pWritebuffer)
{
    ...
    uint8_t *pWBuff = fxls8974_spiWrite_CmdDataBuffer;
    uint8_t *pRBuff = fxls8974_spiWrite_CmdDataBuffer + size + FXLS8974_SPI_CMD_LEN;

    /* Formatting for Write command of FXLS8974 SENSOR. */
    ...
    *(pWBuff + 1) = 0x00;
    ...
    memcpy(pWBuff + FXLS8974_SPI_CMD_LEN, pWritebuffer, size);
...
int32_t FXLS8974_SPI_Initialize(
    fxls8974_spi_sensorhandle_t *pSensorHandle, ARM_DRIVER_SPI *pBus, uint8_t index, void *pSlaveSelect, uint8_t *whoami)
{
    int32_t status;
    uint8_t reg;
    ...
    pSensorHandle->slaveParams.pReadPreprocessFN = FXLS8974_SPI_ReadPreprocess;
    pSensorHandle->slaveParams.pWritePreprocessFN = FXLS8974_SPI_WritePreprocess;
    pSensorHandle->slaveParams.pTargetSlavePinID = pSlaveSelect;
    pSensorHandle->slaveParams.spiCmdLen = FXLS8974_SPI_CMD_LEN;
    pSensorHandle->slaveParams.ssActiveValue = FXLS8974_SS_ACTIVE_VALUE;
...
int32_t FXLS8974_SPI_Deinit(fxls8974_spi_sensorhandle_t *pSensorHandle)
{
    ...
    status = Register_SPI_Write(pSensorHandle->pCommDrv, &pSensorHandle->deviceInfo, &pSensorHandle->slaveParams,
                                FXLS8974_SENS_CONFIG1, FXLS8974_SENS_CONFIG1_RST_RST, FXLS8974_SENS_CONFIG1_RST_MASK);
    if (ARM_DRIVER_OK != status)
    {
        return SENSOR_ERROR_WRITE;
    }
